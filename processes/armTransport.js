...
const handleChange = () => {
  ...
  if (selectIsPlaying(state)) {
    ...
    const fourSteps = async () => await setTimeout4Times(advanceOneTick, duration);
    const sixteenSteps = async () => await setTimeout4Times(fourSteps, duration * 4);
    const sixtyFourSteps = async () => await setTimeout4Times(sixteenSteps, duration * 16);
    const twoHundredFiftySixSteps = async () => await setTimeout4Times(sixtyFourSteps, duration * 64);
